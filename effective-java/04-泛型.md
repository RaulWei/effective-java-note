...
1. 类的类型是`Class<T>`，如`String.class`属于`Class<String>`类型，当它用来传达编译时和运行时的类型信息时，称为`type token`
2. `Map<?, Object>`表示该`Map`的键为某种类型，而`Map<Class<?>, Object>`表示该`Map`的每个键可以有不同的参数化类型
3. `favorites`不能保证键和值之间的类型关系
4. `cast`方法是`cast`操作符的动态模拟
